...
export interface SearchEUImplementationsInput {
  ...
  year_to?: number;
  community?: string;
  has_luxembourg_implementation?: boolean;
  limit?: number;
}
...
export interface SearchEUImplementationsResult {
  results: Array<{
    ...
    luxembourg_statute_count: number;
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS luxembourg_statute_count,
  ...
  if (input.has_luxembourg_implementation !== undefined) {
    sql += input.has_luxembourg_implementation ? ` HAVING luxembourg_statute_count > 0` : ` HAVING luxembourg_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    luxembourg_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        luxembourg_statute_count: r.luxembourg_statute_count,
